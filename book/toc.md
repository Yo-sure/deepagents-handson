...
입문은 개념과 관찰 중심이며, 구현이 있는 장은 **개념 → 실습 → 풀이 → Wrap** 순서로 읽습니다. 실습 안내는 해당 장 안에 있으며, Jupyter의 같은 `notebooks/build-agent.ipynb`를 이어서 완성합니다. 교재의 Python 실행 칸은 작은 동작을 확인하는 용도입니다. [실습 전체 모아보기](./workshop/build)는 복습할 때 사용합니다.

## 교육 목표를 어디서 확인하나요?

[시작 안내의 전체 목표](./workshop/start#course-goals)에서 출발합니다. 각 장 첫머리의 목표를 읽고, 실습 출력과 Wrap에서 달성 여부를 확인합니다. [실습 모아보기의 단계표](./workshop/build)는 노트북 위치와 완료 기준을 연결합니다. 마지막 통합은 구현과 요구 변경을, 20개 퀴즈는 개념 설명을 확인합니다.

## 하루 학습 흐름

아래 시각은 예상 진행 기준입니다. 각 장에는 개념 설명과 실습의 예상 시간이 표시되어 있습니다. ‘생각거리 +3분’과 추가 읽기는 선택 활동입니다. 설치 시간은 환경에 따라 달라질 수 있습니다.
