...
브리프를 내가 쓰고 내가 검증하면 한쪽으로 치우친다. 다른 프로세스, 다른 팀의
에이전트에 검증을 맡긴다. 그 경계가 A2A다. 이 검증 에이전트는 과정에서 '제공 모듈'로
주어진다. 재현성을 위해 LLM 없이 규칙으로 동작한다.
...
Agent Card는 /.well-known/agent-card.json 에 노출된다(에이전트의 서명 가능한 자기소개).
...
def verify_brief(brief_text: str) -> tuple[bool, list[str]]:
    ...
    missing = [name for name, _ in real_gaps if name.split("(")[0] not in brief_text]
    notes = [f"독립 재계산: 영수증 없는 거래 {len(real_gaps)}건 "
             f"({', '.join(f'{n} {a:,.0f}원' for n, a in real_gaps)})"]
    if missing:
        notes.append(f"브리프가 누락한 항목: {', '.join(missing)} — 보완 필요")
        return False, notes
