...
에이전트가 파일과 메일 데이터에 접근하는 인터페이스를 MCP로 표준화한다. 이 과정의 외부 연결은
둘로 고정한다.
  - 실제 파일 : classified/·knowledge_base/ 를 실제로 읽는다.
  - 샘플 메일 : 외부 메일 서버 없이 고정 샘플 목록을 돌려준다.

FastMCP 데코레이터 규칙
  - 함수 이름 → Tool 이름, docstring → 설명(LLM이 보고 호출 판단), 타입힌트 → 입력 스키마.

실행(stdio 대기. 클라이언트가 subprocess로 연결):
...
# 샘플 메일 목록(mock). 외부 메일 서버 대신 이번 달 인박스에 도착한 문서 목록.
MOCK_ENVELOPE = [
...
def list_classified() -> str:
    """분류된 레코드(classified/*.json) 목록을 돌려준다. [실제 파일]"""
...
def _list_tools() -> None:
    ...
    for t in tools:
        tag = "[실제 파일]" if t.name != "fetch_inbox" else "[샘플 메일]"
...
def _show_protocol() -> None:
    ...
    뽑는다. 즉 클라이언트가 이 서버와 stdio로 교환하는 메시지와 같은 내용이다.
    """
    import asyncio

    print("MCP 통신 = stdio(표준입출력) 위 JSON-RPC 2.0. 한 줄에 한 메시지.")
    print("클라이언트가 이 서버를 subprocess로 시작하고 다음 순서로 메시지를 교환한다:")
    ...
    print("\n④ tools/call 요청(모델 출력이 도구 호출을 선택하면 어댑터가 전송):")
