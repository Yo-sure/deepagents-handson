...
5. "대응 문서 없음", "영수증 없음", "미확인" 판정이 레코드와 충돌해 보이면
   `ch4-skills-mcp/reconcile-rules/SKILL.md`의 대사 검증 규칙을 읽고 적용한다.
6. 아래 형식으로 `workspace/brief.md`를 쓴다. 한 화면을 넘기지 않는다.
7. 숫자는 반드시 레코드나 OKF 머리말에서 가져온다. 지어내지 않는다. 근거가 없으면 "확인 필요"로 남긴다.
...
- 읽는 범위는 `workspace/classified/*.json`, `workspace/knowledge_base/*.md`,
  `workspace/research_notes/*.md`, 필요할 때 `references/brief_format.md`와
  `ch4-skills-mcp/reconcile-rules/SKILL.md`로 제한한다.
