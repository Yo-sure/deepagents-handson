...
실행:
    ...
    uv run python3 ch6-integration/analyst_app.py --mock --a2a     # 검증만 실제 A2A
...
def run_okf() -> int:
    ...
    from okf_store import build_finding_entries, build_merchant_entries
...
def write_brief() -> None:
    """Ch4 skill 절차 — 레코드와 OKF 지식을 모아 한 장짜리 brief.md."""
    from analyst.paths import KNOWLEDGE_BASE
    ...
    for p in sorted(KNOWLEDGE_BASE.glob("*.md")):
        ...
        if "type: gap" in t or "type: subscription" in t:
            head = t.split("# ", 1)[1].splitlines()[0]
            kind = "gap" if "type: gap" in t else "subscription"
            flags.append(f"- ({kind}) {head}")
    lines = [
        "# 인박스 브리프 — 2026년 5월", "",
        "## 한 줄 요약",
...
def run_verify(use_a2a: bool) -> None:
    ...
    if use_a2a:
        import asyncio
        proc = subprocess.Popen([sys.executable, str(ROOT / "ch5-a2a" / "verifier_agent.py")])
        try:
...
def main() -> None:
    ...
    ap.add_argument("--mock", action="store_true", help="추출·분류를 gold로(키 없이)")
