...
def corrected_router(state):
    return (
        "draft"
        if state.get("policy_id") and state.get("contact", "").strip()
        else "ask"
    )
...
def test_engineering_reference_preserves_baseline_and_stops_stall():
    assert all(
        ok
        for _, ok in evaluate(
            corrected_router, extension_solutions.refine_without_stall
        )
    )
