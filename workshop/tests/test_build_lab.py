...
@pytest.mark.parametrize("topic,expected", [("정산", "P-01"), (" 계정 ", "P-02"), ("없는업무", None), ("", None)])
...
def test_agent(impl):
    ...
    messages = agent.invoke({"messages": [{"role": "user", "content": '{"topic":"계정"}'}]})["messages"]
    assert any(m.type == "tool" for m in messages)
    assert "P-02" in messages[-1].content

@pytest.mark.parametrize("topic,contact,expected", [("계정", "user", "passed"), ("정산", "   ", "ask"), ("없는업무", "user", "ask")])
def test_graph(impl, topic, contact, expected):
    ...
    def refine(draft, data, limit):
        calls.append("refine")
        return {"draft": draft, "status": "passed", "history": [{"attempt": 0, "draft": draft, "feedback": []}]}
    ...
    assert result["visited"] == (["lookup", "draft", "review"] if expected == "passed" else ["lookup", "ask"])

@pytest.mark.parametrize("kind,limit,status,count", [("good", 0, "passed", 0), ("bad", 0, "held", 0), ("same", 3, "stalled", 1), ("repair", 1, "passed", 1), ("change", 2, "held", 2)])
def test_loop(impl, kind, limit, status, count):
    ...
    def revise(text, feedback):
        ...
        calls.append(text)
        return "재무지원팀 P-01" if kind == "repair" else text + "!" if kind == "change" else text
...
def test_mcp(impl):
    ...
    async def check():
        async with Client(impl.build_mcp_server(impl.lookup_policy), mode=PROTOCOL) as client:
...
@pytest.mark.parametrize("state,artifact,expected", [
    ("working", None, "pending"), ("failed", None, "held"),
...
@pytest.mark.parametrize("request_id,version,artifact", [
