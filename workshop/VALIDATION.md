...
- 기존 코드 계약 검사 83개, 노트북 검사 10개, 키 설정 검사 2개로 학생 완료 검사의 오류 탐지 검사 8개를 더해 총 103개를 통과했습니다.
...
추가로 MCP 이후 앞 장 Agent 재실행과 다른 업무의 초안 수정이 현재 정책을 사용하는지 검사했습니다. 새 완료 검사는 잘못된 정책 반환·공백 처리·불필요한 재질문·검토 실패 무시 등의 잘못된 구현을 거절합니다. 개편한 전체 풀이 노트북은 Gemini 3.8 Flash 실제 호출로도 완료했습니다.
