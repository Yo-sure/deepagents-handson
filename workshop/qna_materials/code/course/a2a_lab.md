    ...
    async def execute(self, context, event_queue: EventQueue):
        ...
        try:
            ...
            await updater.failed(
                message=updater.new_agent_message(parts=[Part(text="요청 형식 오류")])
            )
    ...
    async with asyncio.timeout(60), httpx.AsyncClient(timeout=50) as http:
        ...
        return {
            ...
            "decision": accept_result(
                state, artifact, payload["request_id"], payload["version"]
            ),
        }
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        create_app(args.port), host="127.0.0.1", port=args.port, log_level="warning"
    )
