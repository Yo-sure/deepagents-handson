...
[통합 장](./wrap)에서 퀴즈로 연결 원리를 복습한 뒤, 노트북 6A~6C로 문의 하나를 끝까지 처리합니다. 이어서 6D의 Agent에 MCP 조회와 A2A 발견·위임 도구를 연결하고 정책 검토와 표현 검토 요청을 비교합니다.

<mark class="key-point">정상 처리·추가 질문·보류가 각각 어떤 조건으로 결정됐는지 실행 기록으로 설명합니다.</mark> 정책 근거, 방문 경로, 초안, 원격 검토 결과를 차례로 대조합니다. 정답과 해설은 `build-agent-solution.ipynb`의 같은 번호에서 확인합니다.
