원본: books/workshop/langchain.md
...
|조회 함수를 도구로 만들고 Agent에 연결합니다.|build-agent.ipynb 1A·1B·1C에서 Agent 구성·호출과 복합 도구 사용을 확인합니다.|
...
노트북 **1C. 두 도구를 조합하는 Agent**를 구현합니다. 1A는 준비 운동 3분, 1B는 도구 변환·등록과 지침 구성 12분으로 진행하고, 이 개인 시간에는 정책 결과를 연락처 도구의 입력으로 이어 쓰는 Agent를 만듭니다.

1. **구현 7분:** `build_service_agent`에서 두 도구 변환·등록과 지침을 작성합니다. `ask_service`에서 messages 요청·invoke·전체 결과 반환을 직접 작성합니다.
2. **실행 4분:** 계정과 정산의 근거·담당 팀·이메일을 함께 묻습니다. `lookup_policy` 결과의 팀 이름을 어떤 `lookup_team_contact` 요청이 사용했는지 찾습니다.
3. **실패 분석 4분:** 없는 업무나 거짓 근거·연락처 요청을 하나 골라 실행합니다. 지침을 수정해 같은 질문으로 비교하고, 요청·결과·답변 중 처음 어긋난 지점을 적습니다.

연락처는 학습용 `example.test` 데이터입니다. 실제 발송하지 않습니다. **완료 기준:** 두 정책과 두 연락처가 실제 도구 결과에 근거하고, 없는 값은 추측하지 않습니다. 도구가 모두 호출됐다는 사실만으로 답변까지 정확하다고 판정하지 않습니다. [Agent 구성 공식 문서](https://docs.langchain.com/oss/python/langchain/agents)

### 1B 추가 질문은 1C의 반례로도 사용합니다
...
**막히면 JupyterLab에서 `notebooks/build-agent-solution.ipynb`를 열고 같은 1A·1B·1C 번호를 찾습니다.** 1C에는 두 도구의 등록과 직접 호출 코드, 이어지는 결과 해석 표가 있습니다. 정책 조회 → 팀 이름 선택 → 연락처 조회 → 답변이라는 근거 흐름을 자신의 trace와 대조합니다. 1A·1B의 `lookup_policy`와 `build_agent`도 같은 번호에서 비교합니다. 코드가 비슷한지보다 아래 입력에서 무엇이 실행되고 어떤 근거가 남는지 설명합니다.
