...
<section class="slide" id="qna">

## 질문을 바꿔 묻는 DeepAgents Q&A

`notebooks/qna.ipynb`를 열고 준비 셀을 한 번 실행한 뒤 질문 셀의 `question`만 바꿉니다. 교재·실습·풀이를 검색하고 본문을 읽어 근거 경로와 함께 설명합니다. 예를 들어 “2B에서 visited가 왜 덮어써지나요?” 또는 “6D의 Card 발견과 선택은 누가 하나요?”를 묻습니다. 앞 장의 구현 없이 독립적으로 실행할 수 있습니다.

답변의 근거와 자신의 실행 결과를 대조하십시오. 마지막 선택 셀에서는 검색·읽기 도구의 실제 요청과 응답을 볼 수 있습니다. 수업 고정 자료를 읽으며 웹의 최신 문서를 검색하거나 사용자 코드를 실행하는 기능은 없습니다.
