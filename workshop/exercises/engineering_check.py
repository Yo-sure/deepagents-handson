...
def evaluate(router, refiner):
    ...
    for contact, expected in [
        ...
        actual = build_graph(router=router).invoke(
            {"topic": "정산", "contact": contact}
        )
        results.append(
            (f"회신 대상 {contact!r}: {expected}", actual["decision"] == expected)
        )
    missing = build_graph(router=router).invoke(
        {"topic": "없는업무", "contact": "user"}
    )
    ...
    results.append(
        (
            "수정 함수에 구체적인 실패 이유 전달",
            bool(calls)
            and all(feedback == verify("초안", "정산") for feedback in calls),
        )
    )
    calls.clear()
    good = refiner("재무지원팀 P-01", "정산", unchanged, 0)
    results.append(
        ("최초 성공은 수정 없이 passed", good["status"] == "passed" and not calls)
    )
    held = refiner("초안", "정산", unchanged, 0)
    results.append(
        ("예산 0의 실패는 수정 없이 held", held["status"] == "held" and not calls)
    )
    results.append(
        (
            "성공 초안과 검토 기록을 보존",
            good.get("draft") == "재무지원팀 P-01"
            and good.get("history")
            == [{"attempt": 0, "draft": "재무지원팀 P-01", "feedback": []}],
        )
    )
    results.append(
        (
            "정체 시 실제 두 검토 기록을 보존",
            stalled.get("draft") == "초안"
            and stalled.get("history")
            == [
                {"attempt": i, "draft": "초안", "feedback": verify("초안", "정산")}
                for i in range(2)
            ],
        )
    )
    ...
    results.append(
        (
            "바뀌어도 계속 실패하면 예산에서 보류",
            budget.get("status") == "held"
            and len(changed) == 2
            and budget.get("draft") == "초안!!",
        )
    )
    results.append(
        (
    ...
    results.append(
        (
            "마지막 허용 수정에서 성공하면 passed",
            success.get("status") == "passed"
            and success.get("draft") == "재무지원팀 P-01"
            and len(repaired) == 2,
        )
    )
    results.append(
        (
            "각 수정에 해당 초안의 피드백 전달",
            repaired == [verify("초안", "정산"), verify("담당 팀 확인 중", "정산")],
        )
    )
    return results
