...
def evaluate(module, lab):
    ...
    if lab == "langchain":
        check("조회 성공: 팀과 근거", lambda: all(s in module.answer_from_policy(json.loads(lookup_policy("정산"))) for s in ["재무지원팀", "P-01"]))
        check("없는 규정: 추가 확인", lambda: "확인" in module.answer_from_policy(json.loads(lookup_policy("없는업무"))) and "완료" not in module.answer_from_policy(json.loads(lookup_policy("없는업무"))))
        check("다른 정책도 일반화", lambda: "P-02" in module.answer_from_policy(json.loads(lookup_policy("계정"))))
    elif lab == "graph":
        for topic, contact, expected in [("정산", "user", "draft"), ("정산", "", "ask"), ("없음", "user", "ask")]:
            check(f"{topic}/{contact or '회신대상없음'} → {expected}",
                  lambda t=topic, c=contact, e=expected: build_graph(module.route_inquiry).invoke({"topic": t, "contact": c})["decision"] == e)
    elif lab == "harness":
        for passed, count, limit, expected in [(True, 0, 2, "finish"), (False, 0, 2, "revise"), (False, 2, 2, "hold"), (True, 2, 2, "finish"), (False, 0, 0, "hold")]:
            check(f"pass={passed}, 수정={count}/{limit} → {expected}", lambda p=passed, c=count, l=limit, e=expected: module.loop_action(p,c,l)==e)
    elif lab == "mcp":
...
def main():
    ...
    print("기준 풀이 검사" if args.solution else "학생 코드 검사 (초기에는 실패가 정상입니다.)")
