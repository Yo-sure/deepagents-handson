...
def run(impl, stage="workflow", topic="정산", contact="user@example.test", limit=2, model=None):
    ...
    if remote:
        ...
        def lookup_policy(topic: str) -> str:
            ...
            return json.dumps(data if topic.strip() in snapshot_topics else
                              {"found": False, "topic": topic, "policy": None}, ensure_ascii=False)
    ...
    def revise(draft, feedback):
        response = model.invoke("조회 규정을 근거로 한국어 초안을 수정하십시오.\n" + json.dumps(
                {"policy": json.loads(lookup(topic)), "draft": draft, "feedback": feedback}, ensure_ascii=False))
        return response.content
    def refine(draft, data, limit):
        if stage == "graph":
            issues = inspect_draft(draft, data)
            return {"status": "held" if issues else "passed", "draft": draft,
    ...
    if stage == "complete" and state["decision"] == "passed":
        ...
        payload = {"topic": state["data"]["topic"], "draft": state["draft"], "request_id": str(uuid.uuid4()), "version": 1}
        with server("course.a2a_lab") as url:
            review = asyncio.run(delegate(url, payload))
        # SDK 수신은 제공 코드, 최종 수용 결정은 학생 코드가 담당합니다.
        review["decision"] = impl.accept_review(review["state"], review["artifact"], payload["request_id"], payload["version"])
...
def main(impl=None):
    ...
    parser.add_argument("stage", choices=["agent", "graph", "workflow", "mcp", "complete"])
    ...
    target = ROOT / "runs" / ("build-" + args.stage + "-" + uuid.uuid4().hex[:8] + ".json")
