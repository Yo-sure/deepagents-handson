...
키·네트워크·모델 접근 오류가 발생하면 오류 원인을 해결한 뒤 재실행합니다. 실제 모델의 도구 요청과 tool 반환, 최종 답변이 있는지 읽습니다. LLM 호출을 고정 답변으로 대체하는 실습 모드는 없습니다.
...
현재 과제까지 포기할 필요는 없습니다. 다만 주 실습은 앞 단계 함수에 의존합니다. student.py를 다른 이름으로 복사해 자신의 작업을 보관한 뒤, **미완성인 앞 단계 함수만** reference.py의 같은 함수 정의로 교체합니다. 현재 풀어야 할 함수는 그대로 둡니다. 제공받은 함수는 제출 기록에 구분합니다.
...
자료 버전과 해시 확인은 RELEASE.md, 검증 범위는 VALIDATION.md를 읽습니다. 사람의 학습 시간과 운영 환경의 안전성까지 이 자료의 실행 검증으로 보장하지 않습니다.
