...
def create_ticket(db: Path, business_key: str, content: str) -> dict:
    ...
    with sqlite3.connect(db) as conn:
        conn.execute("CREATE TABLE IF NOT EXISTS ticket (id INTEGER PRIMARY KEY, business_key TEXT UNIQUE, content TEXT)")
        conn.execute("BEGIN IMMEDIATE")
        existing = conn.execute("SELECT id, content FROM ticket WHERE business_key=?", (business_key,)).fetchone()
        if existing:
            if existing[1] != content:
                raise ValueError("같은 업무 키에 다른 내용이 전달됐습니다.")
            return {"ticket_id": existing[0], "created": False}
        cursor = conn.execute("INSERT INTO ticket(business_key, content) VALUES (?, ?)", (business_key, content))
...
async def query(url: str, topic: str):
    async with Client(url, mode=PROTOCOL) as client:
        ...
        return {"protocol": PROTOCOL, "tools": [t.name for t in catalog.tools],
...
if __name__ == "__main__":
    ...
    make_server(args.db).run(transport="streamable-http", host="127.0.0.1", port=args.port,
