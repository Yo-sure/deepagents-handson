...
def main():
    ...
    for lab in labs:
        if lab == "langchain":
            result = run(args.topic, question=args.question) if args.question is not None else run(args.topic)
        elif lab == "graph":
            ...
            result = bounded_refine("담당 팀을 확인합니다.", args.topic,
                                    lambda draft, feedback: revise_draft(draft, feedback, args.topic), args.revisions)
        elif lab == "deepagent":
    ...
    (output / f"{args.lab}.json").write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
