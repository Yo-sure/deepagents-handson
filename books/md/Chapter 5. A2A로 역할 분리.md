...
```mermaid
flowchart LR
    subgraph MINE["내 프로세스 · 내가 소유하고 내부를 앎"]
        O["오케스트레이터"] -->|"task 도구 호출"| SA["서브에이전트<br/>격리 컨텍스트"]
    end
    O -. "A2A · Agent Card + Task (HTTP)" .-> PEER["피어 에이전트<br/>다른 프로세스·팀 · 내부 불투명"]
```
...
<p><strong>한 발 더 — 인프로세스 위임은 사실 <em>도구 호출</em>입니다.</strong> Ch3에서 오케스트레이터가 서브에이전트에 위임할 때 부른 <code>task</code>는 이름 그대로 <strong>도구</strong>입니다(DeepAgents가 배선하는 <code>StructuredTool</code>, 인자 <code>description</code>·<code>subagent_type</code> — Ch3 <code>--trace</code>의 도구 목록에 <code>task</code>가 있던 게 이것). 즉 메인 모델은 <code>task(subagent_type="card_reconcile", …)</code>를 <em>tool_call</em>로 냅니다. 그래서 <strong>인프로세스 위임 = 도구 호출</strong>(내 루프 안에서), <strong>A2A = 프로토콜</strong>(Card + Task를 프로세스 밖으로). MCP·서브에이전트·A2A가 다 "통신"으로 뭉뚱그려지지만, 앞의 둘은 결국 <em>도구 호출</em>이고 A2A만 프로세스 경계를 넘는 별도 프로토콜이라는 게 진짜 구분선입니다.</p>
</div>
</details>
