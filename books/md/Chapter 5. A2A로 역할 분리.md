...
<p class="section-note" style="margin-top:6px">위상도 정리됐습니다 — A2A는 <strong>v1.0</strong>에 이르렀고 Linux Foundation 산하 중립 거버넌스(MCP·AGENTS.md 등과 함께)로 옮겨가, Python·JS·Java·Go·.NET 다섯 SDK로 구현됩니다. 한동안 경쟁 표준이던 IBM의 <strong>ACP</strong>(Agent Communication Protocol, 2025-03)도 2025-08 A2A로 병합돼 <em>단일 표준</em>으로 합쳐졌습니다. 카드를 서명하는 <strong>Signed Agent Card</strong>가 정식 항목이 된 것도 이 흐름입니다 — 다음 절에서 그 신원·서명 <em>모델</em>을 살펴봅니다. <span style="color:var(--muted)">(서명을 실제로 붙이고 검증까지 켜는 건 이 실습 범위 밖입니다 — 우리 코드는 서명 없는 카드를 만들고 클라이언트도 서명을 검증하지 않습니다. 프로덕션에서 켜는 항목으로 보세요.)</span></p>
...
<p class="section-note" style="margin-top:10px"><strong>한 발 더 — 이 검증의 진짜 구멍.</strong> 재계산으로 '영수증 없는 거래'(쿠팡 89,000·넷플릭스 17,000)를 손에 쥐고도, 브리프가 그걸 <em>짚었는지</em>는 상호명 <strong>존재</strong>만 부분문자열로 봅니다(<code>name.split("(")[0] not in brief_text</code>) — <em>금액은 대조하지 않습니다</em>. 그래서 브리프가 "쿠팡(주) <strong>5원</strong>"이라 틀리게 써도 '쿠팡'이라는 글자만 있으면 PASS가 납니다. 검증자가 89,000을 알면서도 그 숫자를 브리프와 맞춰보지 않는 거죠 — "독립 재계산으로 검증한다"는 주장에 난 가장 큰 틈입니다. 동의어·오타도 마찬가지로 샙니다. 실무 검증자는 여기서 <strong>금액까지 교차대조</strong>(±오차 허용)하거나 LLM 판정으로 보강합니다.</p>
...
<div class="cue-body">규칙을 직접 어겨 봅니다. <span style="color:var(--muted)">(먼저 아래 <strong>핸즈온 ②</strong>의 a·b를 한 번 돌려 정상 <code>PASS</code>를 본 뒤 여기로 돌아오면 흐름이 자연스럽습니다 — 깨진 모습과 정상 모습을 비교하게.)</span> <code>verifier_agent.py</code>의 <code>execute</code>에서 <strong>③ <code>enqueue_event(Task(...))</code> 블록을 주석 처리</strong>한 뒤, 핸즈온 ②와 같은 방법으로 서버를 띄워 검증을 보내세요. 다음 줄 <code>updater.start_work(...)</code>가 <em>등록되지 않은</em> Task를 갱신하려다 <code>InvalidAgentResponseError</code>로 막힙니다. 에러를 확인한 뒤 주석을 풀어 되돌리세요. 이 실습은 "상태 갱신 전 Task 등록" 규칙의 필요성을 보여 줍니다.</div>
</div>
</section>
...
<div class="stack">
<div class="row"><div class="code">0</div><div class="copy"><strong>먼저 — 검증할 브리프를 만들어 둔다(Ch3)</strong><p><code>uv run python3 ch3-deepagents/research_orchestrator.py --mock</code><br><span style="color:var(--muted)">이건 A2A 수업이 아니라 <em>검증할 입력을 준비</em>하는 단계입니다. <strong>키가 있으면</strong> Ch4 <code>skill_agent.py --run</code>(live)으로 만든 <code>brief.md</code>를 Ch5가 우선 보내니 이 단계는 건너뛰어도 됩니다. 아직 브리프가 없을 때만 이 줄로 입력을 보장해 둬요 — <code>--mock</code>이라 키 없이 <code>workspace/brief_draft.md</code>가 생깁니다. (성공 기준: <code>brief_draft.md</code> 생성. <code>classified/</code>가 비어 있으면 <code>(classified 비어 있음 — gold에서 보충…)</code>가 뜨는데 정상이고, 더 자연스럽게 가려면 <strong>Ch2 <code>intake_graph.py</code>를 먼저</strong> 돌려 레코드를 채워 두세요. <code>okf_store.py</code>는 <code>knowledge_base/</code> 적재라 검증 선행으로는 선택입니다.)</span></p></div><div class="store">선행</div></div>
...
<div class="row"><div class="code">c</div><div class="copy"><strong>오프라인 — 네트워크 없이</strong><p><code>uv run python3 ch5-a2a/a2a_verify.py --mock</code><br><span style="color:var(--muted)"><strong>증명:</strong> 검증 <em>로직</em>은 네트워크와 무관하다 — A2A를 떼도 같은 <code>verified_brief.md</code>가 나온다(전송만 바뀜). 성공 기준: 같은 PASS 결과가 네트워크 없이 나온다.</span></p></div><div class="store">목</div></div>
</div>
...
<summary>정답 확인</summary>
<div class="reveal">
<p>① <strong>지우면 <code>NEEDS_REVISION</code></strong>. 검증자는 브리프 문장을 믿지 않고 레코드에서 직접 다시 세기 때문에, 쿠팡 89,000원이 빠진 걸 잡아냅니다("브리프가 누락한 항목: 쿠팡").</p>
<p>② <strong>금액만 틀리게 바꾸면 — 그래도 <code>PASS</code></strong>. 검증자는 상호명 '쿠팡'이 본문에 있는지만 보고 금액은 대조하지 않으니까요(위 "한 발 더"의 그 구멍). 같은 누락이라도 <em>빠뜨리면</em> 잡히고 <em>틀리게 쓰면</em> 새는 겁니다 — 검증의 키가 약하면 어디까지 통과하는지를 직접 만든 false-PASS로 확인하는 게 이 실습의 핵심입니다.</p>
<p>외부 검증의 목적은 작성한 문장이 아니라 원천 데이터를 기준으로 다시 판정하는 것입니다. 다만 ②가 보여 주듯 <em>무엇을</em> 대조하느냐가 검증의 실제 강도를 정합니다.</p>
</div>
</details>
</section>
