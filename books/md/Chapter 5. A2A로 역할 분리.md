...
<p>요약으로 잃기 쉬운 맥락 보존용. 단, 현재 <em>인터랙티브 전용</em>이라 백그라운드 포크는 아직 미제공</p>
</div></div></div>
...
<p class="section-note" style="margin-top:12px"><strong>고르는 기준</strong>: 내가 양쪽을 다 소유하고 도구·메모리를 싸게 나눠 쓰고 싶다 → 인프로세스(요약이면 일반 서브에이전트, 전체 맥락이 필요하면 FORK). 상대가 <em>다른 소유자·벤더·언어·신뢰 영역</em>이다 → A2A. 세 가지는 경쟁이 아니라 같은 위임을 분리 수준만 달리한 것입니다.</p>
...
<div class="panel-body"><div class="list">
<p><strong>MCP는 수직(Agent→Tool), A2A는 수평(Agent↔Agent)</strong>입니다. 검증자에게 브리프를 보내는 이 실습은 <em>가장 단순한 수평 1왕복</em>이고, 본격적인 A2A는 여러 에이전트가 서로 Task를 주고받는 <strong>에이전트 간 위임</strong>입니다 — 예컨대 확인 에이전트가 중요한 메일을 찾으면 알림 에이전트에게 A2A로 넘기는 사슬(§REST vs Task 생애주기 심화의 뉴스룸 그림이 그 확장형).</p>
