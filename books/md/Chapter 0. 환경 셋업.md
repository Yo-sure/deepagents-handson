...
<p class="section-note"><strong>전제</strong>: WSL 터미널에서 레포를 먼저 받아 그 폴더 안에 있어야 합니다 — <code>git clone https://github.com/Yo-sure/deepagents-handson ~/lecture</code> → <code>cd ~/lecture</code>. (전체 순서는 Step 2 끝의 「따라 하기 — 처음 한 번」에 한눈에 정리돼 있습니다.)<br>
런타임은 WSL2(Ubuntu 24.04) · Python 3.12 · uv · Node입니다. setup.sh가 가상환경 생성·의존성 설치·.env 템플릿까지 한 번에 처리하니, 실행한 뒤 키만 채우면 됩니다.<br>
...
<div class="cue-body">레포 폴더(<code>~/lecture</code>) 안에서 이 명령을 실행하세요. 첫 실행은 의존성(deepagents·langchain·langgraph·mcp·a2a-sdk)을 내려받느라 네트워크에 따라 수 분 걸립니다. 끝까지 돌면 프리플라이트 점검표가 뜨고, <code>❌ OPENROUTER_API_KEY</code> 한 줄만 빨갛고 나머지가 모두 ✅면 성공입니다. 키는 다음 단계에서 채웁니다.</div>
</div>
...
<div class="row"><div class="code">5</div><div class="copy"><strong>키 발급 → .env 채우기</strong><p><code>openrouter.ai</code> 가입 → <strong>Keys</strong>에서 키 발급 → <code>.env</code>의 <code>OPENROUTER_API_KEY=</code> 줄 뒤에 붙여넣기. 여기까지가 환경 준비입니다 — 모델을 실제로 한 번 불러 보는 첫 호출은 바로 다음 <strong>Step 3</strong>에서 합니다.</p></div><div class="store">키</div></div>
</div>
</div>
...
<figure><img src="/deepagents-handson/images/inbox/receipt_starbucks.png" alt="스타벅스 영수증" loading="lazy"><figcaption>스타벅스 · 11,500원</figcaption></figure>
<figure><img src="/deepagents-handson/images/inbox/receipt_gs25.png" alt="GS25 영수증" loading="lazy"><figcaption>GS25 · 8,400원</figcaption></figure>
<figure><img src="/deepagents-handson/images/inbox/receipt_taxi.png" alt="택시 영수증" loading="lazy"><figcaption>카카오T 택시 · 14,300원</figcaption></figure>
<figure><img src="/deepagents-handson/images/inbox/receipt_restaurant.png" alt="국밥 영수증" loading="lazy"><figcaption>광화문 국밥 · 27,000원</figcaption></figure>
<figure><img src="/deepagents-handson/images/inbox/receipt_oliveyoung.png" alt="올리브영 영수증" loading="lazy"><figcaption>올리브영 · 38,700원</figcaption></figure>
<figure><img src="/deepagents-handson/images/inbox/invoice_photo.png" alt="용역 청구서 사진" loading="lazy"><figcaption>용역 청구서(사진) · 1,650,000원</figcaption></figure>
</div>
</div>
</div>
