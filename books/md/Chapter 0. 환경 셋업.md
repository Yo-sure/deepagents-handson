...
<p class="section-note"><strong>전제</strong>: WSL 터미널에서 레포를 먼저 받아 그 폴더 안에 있어야 합니다 — <code>git --version || (sudo apt-get update && sudo apt-get install -y git)</code> → <code>git clone https://github.com/Yo-sure/deepagents-handson ~/lecture</code> → <code>cd ~/lecture</code>. (전체 순서는 Step 2 끝의 「따라 하기 — 처음 한 번」에 한눈에 정리돼 있습니다.)<br>
...
<p>학생이 직접 수정할 줄은 <code>OPENROUTER_API_KEY</code> 하나입니다. 실습 패키지가 OpenAI 호환 변수(<code>OPENAI_API_KEY</code>·<code>OPENAI_API_BASE</code>)가 비어 있으면 같은 OpenRouter 키와 <code>https://openrouter.ai/api/v1</code>로 맞춰 재현성을 보장합니다. 기업/강사용으로 다른 OpenAI 호환 게이트웨이를 명시한 경우에는 그 설정을 덮지 않습니다</p>
...
<div class="cue-body">먼저 <code>bash scripts/preflight.sh</code>로 live 점검 <code>✅ 14 / ❌ 0</code>을 확인하세요. 이어서 이 코드를 <code>first_call.py</code>로 저장하고 <code>uv run python3 first_call.py</code>로 실행합니다. <code>.env</code>의 <code>OPENROUTER_API_KEY</code>를 먼저 채워야 합니다. Gemini 3.5 Flash는 과금 모델이라 OpenRouter 계정에 크레딧이나 결제 한도가 없으면 402/credit 계열 오류가 날 수 있습니다.</div>
</div>
