...
<div class="row"><div class="code">2</div><div class="copy"><strong>첫 LLM 호출</strong><p>Gemini 3.1 Flash Lite 라우팅 확인</p></div><div class="store">응답</div></div>
...
<p class="section-note"><strong>대부분의 강의장 PC엔 이미 WSL이 깔려 있습니다.</strong> 먼저 <strong>Windows PowerShell</strong>에서 아래 <code>wsl</code> 한 줄만 쳐 보세요 — 바로 들어가면 끝입니다.</p>

```powershell
# Windows PowerShell에서 — 대개 이 한 줄로 Ubuntu에 바로 들어간다
wsl
#   프롬프트가 …$ 로 바뀌면 성공 → 곧바로 Step 1로.
```

<details>
<summary>안 될 때 — "설치된 배포판이 없습니다" 등이 나오면 (설치부터)</summary>
<div class="reveal">

```powershell
# 1) 설치 상태 확인 — Ubuntu-24.04가 목록에 있나
wsl --list --verbose
#   NAME            STATE     VERSION
#   Ubuntu-24.04    Stopped   2      ← 있으면 설치 건너뛰고 3)으로
...
윈도우와 리눅스 경로가 뒤섞이는 일도 이때 정리됩니다. 한 번만 맞춰 두면 됩니다.</p>
</div>
...
<p class="section-note">기본 실습 모델은 Gemini 3.1 Flash Lite입니다. OpenRouter 게이트웨이로 한 번 불러 보면 키와 경로, 모델 라우팅이 제대로 잡혔는지 30초 안에 확인됩니다.<br>
...
<div class="section-head">
<div>
<div class="eyebrow">Step 4 · 샘플 문서 · 3분</div>
...
<p class="section-note">아래가 실습 내내 모델이 그대로 받아 읽는 입력입니다. 영수증 5장과 사진 청구서 하나입니다. 이 이미지에서 판매처·금액·항목을 뽑아 RecordV1로 채웁니다. (나머지 카드·은행 명세서·계약서·리포트는 PDF라 여기선 생략합니다.)</p>
...
코드가 정본입니다. 교재는 그 파일을 그대로 임베드하며, 따로 복사해 둔 사본이 아닙니다.</p>
</div>
...
<div class="reveal">
<p>RecordV1은 같은 필드에 이름을 두 개 둡니다. 코드에서 쓰는 이름은 <code>merchant</code> 같은 영문이고, 저장되는 JSON의 이름은 <code>판매처</code> 같은 한글입니다. 이렇게 나누는 이유는 간단합니다. <strong>코드에서는 편집기·타입 검사가 다루기 좋은 이름</strong>을 쓰고, <strong>사람이 여는 파일에는 읽기 좋은 이름</strong>을 쓰기 위해서입니다.</p>
<table>
...
<p class="muted"><strong>핵심 정리.</strong> 추출·코드 계약은 영문입니다. 사람이 읽는 저장 파일만 한글입니다. 현지화는 LLM이나 코드 중간에 섞지 않고, 파일로 내보내는 마지막 단계에서만 처리합니다.</p>
</div>
</details>
