...
<p class="section-note">다음 <code>wsl</code> 명령은 <strong>Windows PowerShell</strong>에서 칩니다(관리자 권한 권장). <strong>대부분의 PC는 이미 설치돼 있습니다</strong> — 먼저 <code>wsl</code>만 쳐 보고, 바로 들어가면 1)~3)은 건너뜁니다.</p>

```powershell
# 0) 먼저 이것부터 — 강의장 PC는 대개 이미 깔려 있어 이 한 줄로 Ubuntu에 바로 들어간다
wsl
#   프롬프트가 …$ 로 바뀌면 성공 → 아래 1)~3)은 건너뛰고 Step 1로.
#   "설치된 배포판이 없습니다" 류 메시지가 나오면 아래 1)부터.

# 1) (0이 안 될 때) 설치 상태 확인 — Ubuntu-24.04가 목록에 있나
wsl --list --verbose
#   NAME            STATE     VERSION
