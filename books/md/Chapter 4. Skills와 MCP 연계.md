...
<div class="board-header"><span>Skills의 짝 — AGENTS.md(항상 켜진 지침)</span><span class="status-pill">AGENTS.md · AAIF</span></div>
<div class="panel-body"><div class="list">
<p><strong>Skill = 필요할 때 펼치는 <em>절차</em></strong>(점진 공개). <strong>AGENTS.md = 항상 켜진 <em>프로젝트 지침</em></strong> — 빌드·테스트 명령, 코드 컨벤션, 아키텍처 제약을 담아 에이전트 컨텍스트(시스템 프롬프트 격)에 상시 로드됩니다. 사람용 <code>README</code> 옆에 두는 "에이전트용 README"입니다.</p>
<p>플레인 마크다운(스키마·YAML 없음)이고, Codex·Copilot·Cursor·Windsurf·Zed 등 <strong>28개+ 툴이 공통으로 읽는 크로스툴 표준</strong>입니다(Claude Code의 <code>CLAUDE.md</code>가 같은 역할). 실수를 발견할 때마다 규칙을 한 줄씩 조여 넣는 "실패 이력 규칙서"로 쓰고, 150줄 이하·신호 밀도를 권장합니다.</p>
<p class="tiny" style="color:var(--muted)"><strong>거버넌스.</strong> 이 챕터의 <strong>MCP</strong>와 <strong>AGENTS.md</strong>는 둘 다 2025-12 출범한 <strong>AAIF</strong>(Agentic AI Foundation, Linux Foundation)가 관리합니다(Block의 goose 포함, A2A도 LF 생태계 합류). 특정 벤더가 아니라 공용 재단이 표준을 잡는 쪽으로 정리되는 중입니다.</p>
</div></div>
</div>
...
<p class="tiny" style="color:var(--muted)"><strong>훅이란?</strong> 미들웨어가 에이전트 루프의 <em>정해진 지점</em>에 끼어드는 자리입니다. Ch2에서 본 여섯 — 실행 1회 단위 <code>before_agent</code>·<code>after_agent</code>, 매 모델 호출 전후 <code>before_model</code>·<code>after_model</code>, 호출을 통째로 감싸는 <code>wrap_model_call</code>·<code>wrap_tool_call</code>(캐싱·재시도·결과 가로채기) — 이 그 지점들입니다. <code>SkillsMiddleware</code>는 그중 <strong>세션 시작 1회의 <code>before_agent</code></strong> 하나만 써서 스킬 메타데이터를 싣습니다.</p>
...
<p class="tiny" style="margin-top:12px;color:var(--muted)"><strong>실제 예.</strong> 이렇게 묶은 plugin은 각 도구의 마켓플레이스로 배포됩니다 — 예로 세션 recall·이어가기 등을 묶은 Claude Code 플러그인 <a href="https://github.com/awithi-co/acdc" target="_blank" rel="noopener">ACDC</a>가 있습니다.</p>
</section>
