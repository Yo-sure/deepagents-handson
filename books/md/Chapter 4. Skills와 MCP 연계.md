...
<div class="panel-body"><div class="list">
<p><strong>서버가 노출할 수 있는</strong> 셋 — <strong>Tool</strong>(모델이 자율로 호출, 부수효과 가능) · <strong>Resource</strong>(클라이언트가 읽어가는 읽기전용 데이터) · <strong>Prompt</strong>(사용자가 트리거하는 템플릿). 우리 서버는 이 중 <strong>Tool 4개</strong>와 <strong>Resource 1개</strong>만 구현하고, Prompt는 개념으로만 다룹니다.</p>
...
<p>그 위로 흐르는 메시지는 <strong>JSON-RPC 2.0</strong>입니다. 여기서는 <code>tools/list</code>로 도구를 발견하고, <code>tools/call</code>로 하나를 실행하는 흐름만 보면 됩니다.</p>
</div></div>
</div>
...
<div class="cue-body">소스를 고치지 말고 임계값만 바꿔 봅니다. 먼저 임시 워크스페이스에 Ch2 레코드를 만들고, 구독 판정 기준을 올려 OKF를 다시 적재합니다.<br><code>ANALYST_WORKSPACE=/tmp/acdc-ch4-threshold uv run python3 ch2-langgraph-agent/intake_graph.py --mock</code><br><code>ANALYST_WORKSPACE=/tmp/acdc-ch4-threshold ACDC_SUBSCRIPTION_LIMIT=100000 uv run python3 ch4-skills-mcp/okf_store.py</code><br>어떤 항목이 gap에서 subscription으로 바뀔까요?</div>
</div>
...
<p>실무에서는 이런 임계값을 도메인 전문가가 정합니다. <code>ACDC_SUBSCRIPTION_LIMIT</code>이 "구독이냐 확인이냐"를 가르는 정책 그 자체 — 설정값 하나에 회계 판단이 박혀 있다는 뜻입니다.</p>
...
<p>okf_store는 classified 레코드가 필요합니다. Ch2 intake를 먼저 실행해 <code>workspace/classified/</code>에 JSON 10개를 만들어 두세요. 키 없이 순서만 확인할 때는 Ch2를 <code>--mock</code>으로 돌리면 됩니다.</p>
</div></div></div>
