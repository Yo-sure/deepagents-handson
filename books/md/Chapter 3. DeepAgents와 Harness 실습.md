...
<details class="deep">
<summary>🔬 심화 · <strong>강의용</strong> — 공개 백엔드와 셸 실행·샌드박싱 <span style="color:var(--muted)">(현재 deepagents 공개 API 기준)</span></summary>
<div class="reveal">
...
<p class="muted"><strong>가르칠 때 한 줄</strong> — "긴 작업의 적은 컨텍스트 한도다. 답은 <em>상태를 밖으로 빼는 것</em> — todo/state로 진행을 관리하고, 큰 산출물은 파일·스토어에 두면 감사와 재시도가 쉬워진다." <code>git revert</code>·<code>init.sh</code>·<code>progress.txt</code>는 특정 API가 아니라 그 패턴을 구현하는 흔한 도구다.</p>
</div>
</details>
...
<details class="deep">
<summary>🔬 심화 · <strong>강의용</strong> — 루프 엔지니어링: 하네스 다음 단계 <span style="color:var(--muted)">(2026 동향)</span></summary>
<div class="reveal">
...
<p class="muted"><strong>가르칠 때 한 줄</strong> — "프롬프트→컨텍스트→하네스→루프. 오늘 짓는 하네스 위에 '반복 실행되는 순환'을 얹는 게 다음 단계이고, 그 순환의 품질은 <em>검증자</em>가 정한다." 학생에겐 ①~④ 한 줄과 "병목은 검증자"면 충분하다.</p>
...
```text
▶ 조사 대상 10건
  [live] create_deep_agent → task 서브에이전트 3개 위임 요청
  ...
...
<p>키를 넣으면 <code>create_deep_agent</code>가 실제로 추론하며 도구를 부르므로 수십 초 걸립니다. 구조 확인은 <code>--mock</code>이 빠릅니다.</p>
</div></div></div>
