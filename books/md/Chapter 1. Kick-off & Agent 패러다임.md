...
<p>이진 채점이 "자신 있는 추측"을 보상해 습관이 남습니다(Kalai 외 2025). 모르면 0점이라 일단 찍죠.</p>
...
<div class="stack">
<div class="row"><div class="code">S</div><div class="copy"><strong>Stateless — 캐시하고, 요약은 의심하고, 메모리를 붙인다</strong><p>호출 사이에 상태가 없으니 같은 맥락을 매번 다시 보냅니다. 대응은 세 갈래입니다.</p>
...
<div class="board" style="margin-top:18px">
<div class="board-header"><span>①과 ②는 충돌한다 — 캐시 vs 압축</span><span class="status-pill">트레이드오프</span></div>
<div class="panel-body"><div class="list">
<p>프리픽스 캐시는 <strong>앞부분이 글자 그대로 같을 때만</strong> 적중합니다. 그런데 ②요약은 바로 그 앞부분을 <strong>새로 고쳐 쓰는 일</strong>이라, 요약하는 순간 캐시가 깨지고 다시 풀가(+새 캐시 쓰기 비용)로 냅니다. 둘은 정면으로 당깁니다 — 캐시는 "프리픽스를 얼리라", 압축은 "프리픽스를 바꿔라".</p>
<p>숫자로 따지면 — 캐시 <strong>읽기는 원가의 ~10%</strong>(Anthropic. OpenAI는 50%~최대 90%↓), 쓰기는 1.25×(5분 TTL)~2×(1시간). 같은 프리픽스를 TTL 안에 <strong>2회만 재사용해도 본전 이상</strong>입니다. 반대로 매 턴 요약해 버리면 그 이득이 통째로 날아가고 쓰기 비용만 쌓입니다.</p>
<p>그래서 실무 절충은 둘을 분리합니다 — 안 바뀌는 부분(시스템 프롬프트·고정 문서)은 <strong>캐시로 얼리고</strong>, 요약은 휘발성 꼬리에만 <strong>가끔</strong> 겁니다. 갈림길 질문은 하나: <em>"이 프리픽스를 TTL 안에 다시 쓰나?"</em> — 자주 쓰면 캐시, 창에 안 들어가거나 재사용이 없으면 압축.</p>
</div></div>
</div>
</section>
...
<div style="display:grid;grid-template-columns:60px 1fr 54px;align-items:center;gap:10px;font-weight:800"><span>식비</span><span style="display:block;height:18px;border-radius:5px;background:var(--forest);width:98.6%"></span><span style="text-align:right">98.6%</span></div>
<div style="display:grid;grid-template-columns:60px 1fr 54px;align-items:center;gap:10px;font-weight:800"><span>생활</span><span style="display:block;height:18px;border-radius:5px;background:var(--blue);width:1.0%;min-width:6px"></span><span style="text-align:right">1.0%</span></div>
<div style="display:grid;grid-template-columns:60px 1fr 54px;align-items:center;gap:10px;font-weight:800"><span>교통</span><span style="display:block;height:18px;border-radius:5px;background:var(--teal);width:0.3%;min-width:4px"></span><span style="text-align:right">0.3%</span></div>
</div>
