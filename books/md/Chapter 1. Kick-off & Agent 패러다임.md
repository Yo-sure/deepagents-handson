...
<div class="reveal">
<p>① 명세서는 항목 합계가 총액과 맞아떨어지는 영수증 구조가 아닙니다. <code>extract_react</code>에 <em>doc_type을 보는 강제 분기는 없고</em>, 검산이 의미 없다고 <strong>모델이 판단해</strong> <code>check_receipt_sum</code>을 부르지 않은 채 바로 최종 JSON을 냅니다. "무엇을·언제 검증할지를 모델이 정한다"는 ReAct의 성질이 그대로 드러나는 대목입니다(강제 if 문이 아니라 모델 판단).</p>
