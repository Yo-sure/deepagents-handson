...
노트북 ‘4. MCP’의 `build_mcp_server`를 작성합니다. MCPServer를 만들고 받은 policy_tool을 등록한 뒤 서버를 반환합니다. 정의 셀 다음의 연결 셀에서 `topic`을 정하고 실행합니다.

실제로 HTTP 서버를 시작해 목록과 결과를 받은 뒤 종료합니다. 셀 출력의 도구 목록에 lookup_policy, 결과에 P-02·IT지원팀이 있으면 정상 조회입니다. 없는업무도 실행해 found=False를 확인합니다. **별도 터미널에서 서버를 띄우거나 함수를 파일로 옮기지 않습니다.**
