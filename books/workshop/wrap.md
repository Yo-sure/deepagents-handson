...
처음에는 “이 답변이 어디서 나왔는가”를 실행 기록으로 확인했습니다. 이제는 자신이 작성한 코드에서 그 근거가 어떻게 전달되는지 설명할 차례입니다. 마지막 별칭 변경은 새 기능을 많이 붙이는 활동이 아니라, <mark class="key-point">요청의 표현이 바뀌어도 같은 정책과 검토 계약을 유지할 수 있는지 확인하는 활동입니다.</mark>
...
|새 업무 표현을 지원하면서 기존 동작을 유지합니다.|로그인 별칭을 추가하고 계정·정산·미등록 업무를 다시 확인합니다.|
...
공통 도달점은 조회 도구·LangChain Agent 구성·업무 분기·추가 질문 노드·MCP 등록·A2A 수용 조건의 구현과 실행 결과 해석입니다. 제공 Graph의 상태·노드·간선을 읽고, 제공 루프의 성공·정체·예산 종료를 설명합니다. 요청·버전 검사는 이 실습이 A2A 산출물에 추가한 업무 계약입니다. Loop·Graph Engineering의 실제 담론과 활용 활동에서는 반복 작업의 시작·선택·종료 조건과 역할별 의존성·산출물 계약을 정의합니다. 전체 그래프·루프 재구현, 영속 실행 복구·동시성·권한은 선택 심화입니다. 이 하루만으로 운영 수준의 Agent 시스템을 완성했다고 보지는 않습니다.
...
## Wrap · 오늘 배운 개념 20개를 퀴즈로 복습합니다

<p class="section-time">예상 8분 · 17:52–18:00</p>

답을 하나 고르면 정답과 이유가 나타납니다. 헷갈린 문제는 해당 개념으로 돌아가 확인한 뒤 다시 풀어 봅니다. 20개를 마치면 틀린 문제만 모아 재도전할 수 있습니다. 새로고침하면 풀이 기록이 초기화됩니다.
