...
## 시작 질문 · 답변이 나왔는데, 규정은 읽었을까요?

<p class="section-time">예상 3분 · 10:40–10:43</p>

정산 문의에 Agent가 “재무팀으로 연락하세요”라고 답했습니다. 문장은 자연스럽지만, 사내 규정을 조회했는지는 아직 모릅니다.

**담당 팀을 추측한 답변과 실제 조회한 답변을 어떻게 구분할까요?**

LangChain의 최근 글은 모델과 도구를 연결하는 실행 루프와 결과를 확인하는 검증 루프를 구분합니다. 실행된다는 것과 올바르게 처리한다는 것은 다릅니다. [2026-06-16 · 개발사 기술 해설](https://www.langchain.com/blog/the-art-of-loop-engineering) · [GeekNews 소개](https://news.hada.io/topic?id=31106)

이번에는 조회 함수를 직접 만들고 Agent에 연결합니다. 실행 뒤에는 도구가 받은 입력과 반환한 정책을 확인합니다.
...
“답변이 그럴듯하다”는 의견에는 “조회 기록 없이도 그렇게 말할 수 있을까요?”라고 되묻습니다. create_agent가 검증을 전혀 지원하지 않는다는 뜻은 아닙니다. 무엇을 정답으로 판정할지는 개발자가 정해야 한다는 데 초점을 둡니다.
