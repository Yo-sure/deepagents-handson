...
이 장의 목표는 **도구 호출 요청과 실행 결과를 구분하고, 그 결과에 따라 Agent의 다음 행동이 왜 달라지는지 설명하는 것**입니다. 새 코드를 작성하기 전에 제공된 예제의 실행 기록을 읽습니다. 다른 연구와 Harness는 이후 배울 내용을 연결하며 간단히 살펴봅니다.
...
셀에는 실행을 읽기 쉽게 정리한 메시지가 있습니다. 모든 설정·네트워크 요청·시스템 지시를 빠짐없이 담은 원본 통신 기록은 아닙니다. 지시문은 `course/langchain_lab.py`의 `system_prompt`에서 따로 확인합니다. 지금은 코드를 수정하지 않습니다.
