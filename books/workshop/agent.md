...
## 시작 질문 · 검색 결과가 예상과 다르면 어떻게 할까요?

<p class="section-time">예상 3분 · 09:40–09:43</p>

질문을 받은 프로그램이 검색하고 답변합니다. 검색 결과에 필요한 내용이 없다면 검색어를 바꿀까요, 다른 도구를 쓸까요, 사용자에게 다시 물을까요?

**이 다음 행동을 코드에 미리 정해 두는 경우와 모델이 결과를 보고 고르는 경우는 어떻게 다를까요?**

ReAct 연구는 추론과 행동을 번갈아 수행하며 외부에서 얻은 관찰을 다음 판단에 사용하는 접근을 제안했습니다. 오늘은 이 실행 방식을 중심으로 Agent를 배웁니다. [ReAct 원논문 · ICLR 2023](https://react-lm.github.io/)

<details class="instructor-note"><summary>강사용 진행 노트 · 시작 질문</summary>

검색 실패 뒤 가능한 행동을 두 가지 받아 적고, 누가 선택하는지 묻습니다. 연구 발표 연도나 약어를 외우게 하기보다 관찰에 따라 행동이 달라지는 상황으로 연결합니다.
...
**함께 생각하기:** 규정 조회 결과가 ‘지역별 한도’, ‘일률 한도’, ‘조회 오류’일 때 다음 행동을 각각 골라 봅니다. 무엇을 모델이 선택하고 무엇을 코드로 제한해야 할까요?

<aside class="discussion-prompt"><strong>생각거리 · 여유가 있으면 +3분</strong><p>두 프로그램이 같은 답을 냈습니다. 하나는 정해진 순서대로 조회했고, 다른 하나는 모델이 조회 여부를 골랐습니다. 최종 답변만 보고 둘을 구분할 수 있을까요?</p></aside>

<details class="instructor-note"><summary>강사용 토론 길잡이</summary>

출력만으로는 실행 방식을 판별하기 어렵습니다. 어떤 결정을 모델에 맡겼는지와 실행 기록을 봅니다. Agent라는 이름보다 그 선택이 업무에 필요한지도 물어봅니다.
...
**선택 질문:** 도구를 사용할 순서를 미리 계획해야 하는 문제와, 실패한 이유를 다음 시도에도 기억해야 하는 문제는 같을까요? 전자는 ReWOO, 후자는 Reflexion의 질문과 연결해 봅니다. 모든 방법을 구현하기보다 해결하려는 문제가 어떻게 다른지 설명합니다.

<aside class="discussion-prompt"><strong>생각거리 · 여유가 있으면 +3분</strong><p>계획을 먼저 세우는 Agent가 도구에서 예상 밖의 결과를 받았습니다. 계획을 끝까지 실행할까요, 다시 계획할까요? 어느 조건에서 전략을 바꾸겠습니까?</p></aside>

<details class="instructor-note"><summary>강사용 토론 길잡이</summary>

정해진 입력 범위에서는 계획을 따를 수 있지만, 전제가 깨졌다면 재계획이나 보류가 필요합니다. 비용 절감과 적응성 사이의 선택이며, 한 연구 이름이 모든 상황의 답은 아닙니다.
...
Harness는 모델 주변의 컨텍스트·도구·상태·권한·실행 제어를 묶어 말하는 개념입니다. DeepAgents는 이런 기능을 묶어 제공하는 Agent Harness 구현체입니다. ReAct와 경쟁하는 별개의 Agent 종류가 아닙니다. LangChain의 `create_agent`도 기본 실행 루프와 주변 구성을 제공하므로 Harness가 DeepAgents에서 처음 등장하는 것은 아닙니다. [공식 DeepAgents 설명](https://docs.langchain.com/oss/python/deepagents/overview) 최근 Loop Engineering 담론은 사람이 매번 코딩 에이전트에 다음 지시를 쓰던 일을 시스템에 맡기는 방향을 다룹니다. 작업 발견·배정·검증·기록·다음 실행을 설계하는 사례를 [Harness 모듈의 원문 비교](./engineering#distinction)에서 읽습니다.
...
4. ReAct와 ReWOO는 무엇을 다르게 배치하나요? Reflexion은 ReAct와 함께 쓸 수 있나요?
5. DeepAgents를 ReAct와 나란히 놓고 서로 다른 Agent 종류라고 분류해도 될까요?
